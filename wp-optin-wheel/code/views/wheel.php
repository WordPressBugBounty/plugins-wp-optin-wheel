...
<style>
	<?php if($model->bgpattern != 'none'){ ?>
		.wof-wheel[data-id="<?php _e($model->id) ?>"]:after{<?php _e($model->get_background()) ?>}
	<?php } ?>
</style>
...
<div
	...
	<?php _e($model->data_attributes()) ?>
>
	<div class="wof-wrapper">
		<div class="wof-close-wrapper">
			<a class="wof-close wof-fgcolor" href="#">
				<?php _e($model->has_setting('close_text') ? $model->close_text : __('Close')) ?>
			</a>
		</div>
				...
				<div class="wof-left-inner">
					...
					<div class="wof-wheel-container wof-spinning">
						...
						<?php foreach($model->slices as $slice) { ?>
							<div class="wof-slice" data-slice="<?php _e($ctr) ?>" style="transform:rotate(<?php _e($start_degrees) ?>deg) translate(0px, -50%)"><?php _e(wp_strip_all_tags($slice->label)); ?></div>
				...
				<div class="wof-right-inner">
					...
					<?php if($model->has_setting('explainer')){ ?>
						<div class="wof-explainer wof-fgcolor">
							<?php _e($model->explainer) ?>
						</div>
					<?php } ?>
					<div class="wof-form-wrapper">
						<div class="wof-error" style="display: none;"></div>
						<input type="email" data-wof-required="email" class="wof-email" name="wof-email" placeholder="<?php _e($model->setting_or_default('email_placeholder', __('Your email',Config_Manager::$slug))) ?>" />
						<div class="wof-form-fields"></div>
						<button	class="wof-btn-submit wof-color-2" type="submit">
							<span><?php _e($model->setting_or_default('button_text',__('Try your luck',Config_Manager::$slug))) ?></span>
